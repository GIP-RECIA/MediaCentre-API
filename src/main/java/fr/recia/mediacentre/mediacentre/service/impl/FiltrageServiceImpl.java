...
import fr.recia.mediacentre.mediacentre.model.filter.FiltreFavoris;
import fr.recia.mediacentre.mediacentre.model.filter.category.CategorieFiltres;
import fr.recia.mediacentre.mediacentre.model.filter.category.CategorieFiltresCalculees;
...
@Service
@Slf4j
public class FiltrageServiceImpl implements FiltrageService {
	...
	public String preparerFiltrage(
    	...
    	for (CategorieFiltres categorie : categoriesFiltres ) {
    		if ( !this.concerneUtilisateur(categorie, userInfos)) {
    			log.debug("La catégorie de filtre {} ne concerne pas l'utilisateur {}", categorie, userInfos);
    			continue;
    		}
			log.debug("Traitement de la catégorie de filtre {}", categorie);

    		if (categorie.estCategorieCalculee()) {
    			categorie = (CategorieFiltresCalculees)categorie.clone();
    			((CategorieFiltresCalculees)categorie).initialiser(userInfos, ressources);
    		...
    		for (Filtre filtre : categorie.getFiltres()) {
    			if (!this.concerneUtilisateur(filtre, userInfos)) {
					log.debug("Le filtre {} ne concerne pas l'utilisateur {}", filtre, userInfos);
    				continue;
    			}
	 ...
	 */
	public boolean concerneUtilisateur(final FilterUserRight filter, final Map<String, List<String>> userInfoMap) {
